//Successfully adds and bans trades, based on MainNet simulations.
...
var approvalGasPrice = '82000000000'


const { ChainId, JSBI, BigintIsh, Fetcher, WETH, Route, Token, Pair, Price, Trade, Fractions, TokenAmount, TradeType, Percent } = pkg;
const chainId = ChainId.MAINNET;

let wsProvider = new ethers.providers.WebSocketProvider("ws://18.141.143.98:8546",chainId); //Needed for uniswapV2 JS SDK, not compatible with Web3
var productionServer = "ws://18.141.143.98:8546"; //Amazon private GETH MainNet Node
...
const PRIV_KEY = Buffer.from('acc46d27233145b83fe2cdc39c289734e2dd5248df814f88572f0926bd331a3e', 'hex') //for account: 0x343EeF47523000FB24a58511dF30Ed64BBE5a79E
const PRIV_KEY2 = Buffer.from('1eecfff90f504f5f2e74cc258dd89aa8a4590285f96c5490cb519008cd22a906', 'hex')
const PRIV_ACC = "0x33127Db5639fDcB7C4b7B98E4f90773D45a11f08"
const PRIV_ACC2 = "0x59a41C043972Ef71E9117EA47AC0131cB4B700bb"
...
var minProfitAmount = 1 //USD
var minGasPrice = 10 //GWEI - Make sure we wont bid a trade that is super slow
var maxGasPrice = 400 //GWEI - Make sure we wont be totally out of contention
var minGasLimit = 135000 //WEI - Stop TX reverting due to out of gas
var maxTradeRiskSize = 30000 //USD - may do in eth?
...
for (let i = 1; i < 500; i++) { //Simulate 500 different inputs from 1% to 5x of original trade notional, return max PnL amount seen. Returns optimal trade size. Assumes no other front runs or pending exec in block
 ...
 if(isProfitableFlagAfterGas){
  ...
  if (!checkBannedContracts(tokenB.address)){
    isTokenAllowed = true; //Not yet banned!
  }else{
    console.log("Failed: ERC20 is on the banned list - previously failed a simulation")
  }

  if(isAboveMinProfit && capitalRequiredIsUnderMaxRisk && isInReasonableTimeFrame && isGasPriceExploitable && isGasLimitValid && isAboveMinTradeAmount && isTokenAllowed){
    ...
    console.log("$$$$$$$$$$$$")
  //  firstRun = false; //Stop More Execution     //GLOBAL_CAN_TRADE_FLAG = false 
  firstRun = false

    //Build out execution code
    //Build out approval if needed
    var localWeb3 = new Web3(ganacheCLI.provider({fork:'ws://18.141.143.98:8546', port:7535, mnemonic:'hire buyer initial evoke adapt arrange autumn deliver clump rent segment shrug'})) //blockTime: 0.2
    localWeb3.eth.getAccounts(console.log);

    const otherTokenContractLocal = new localWeb3.eth.Contract(ERC20TransferABI, tokenB.address) 
    localWeb3.eth.handleRevert = true

    //Check if Whitelist, if not simulate, but DO not trade
    if(!checkApprovedContracts(tokenB.address) && !checkBannedContracts(tokenB.address)){
      console.log(_symbol, "Not approved or banned. Starting approval simulation - No trade will occur....")
      var myAccount = (await localWeb3.eth.getAccounts())[0] //Alias
      var totalPnL = -1
      var closeBalanceMyETH
        //Simulate a transaction on the first occurence of ticker - if Postive PnL, then add to whitelist else, blacklist
        approveERC20Spend(tokenB.address, UNISWAP_ROUTERV2_ADDRESS, PRIV_ACC, localWeb3, _transaction, _symbol, 0)
        .then(//Custom Token - Me
        createAndSendMyBuyTransactionSimulated(_decodedInput, _transaction, PRIV_ACC, localWeb3, _symbol, lastRunVariables, 1))
        .then(
        createAndSendCopyBuyTransactionSimulated(_decodedInput, _transaction, PRIV_ACC2, localWeb3, _symbol, lastRunVariables, 0)
        ).then(
        createAndSendMySellTransactionSimulated(_decodedInput, _transaction, PRIV_ACC, localWeb3, _symbol, lastRunVariables, 2))
        
        setTimeout(async() => {
            closeBalanceMyETH = await localWeb3.eth.getBalance(myAccount)
            totalPnL = ((localWeb3.utils.fromWei(closeBalanceMyETH,"ether")) -100) * ethValue
            console.log("PNL FROM APPROVAL RUN:", _symbol, String(totalPnL))    

          if(totalPnL > 0.01){
            //approveERC20Spend(tokenB.address, UNISWAP_ROUTERV2_ADDRESS, myAccount, localWeb3, _transaction, _symbol) //Custom Token - Me
            addNewApprovedContract(tokenB.address, _symbol)          
            firstRun = true
            return
          }else{          //Add the ticker to the blacklist
            addNewBannedContract(tokenB.address, _symbol)          
            firstRun = true
            return
        }
    },8000);
  }else{
      console.log(_symbol,"Already Approved")
      firstRun = true
      return
  }
}
...
async function createAndSendMyBuyTransactionSimulated(_decodedInput, _transaction, _myAccount, _localWeb3, _symbol, _lastRunVariables, _nonce){
   ...
   const txParams = {
     gas : _localWeb3.utils.toHex(_transaction.gas), //Copy original tickets gas limit
     gasPrice: _localWeb3.utils.toHex(String(parseInt(_transaction.gasPrice) + 100000000)), //Just bid slightly more than original ticket
     to: UNISWAP_ROUTERV2_ADDRESS,
     data: approvalTxDataABI,
     value: _localWeb3.utils.toHex(String(_lastRunVariables.myETHInput.raw)),
     from: _myAccount,
     nonce: _localWeb3.utils.toHex(_nonce)
   ...
   _localWeb3.eth.sendSignedTransaction('0x' + serializedTx.toString('hex'))
...
//2nd Trade - Other Buy
async function createAndSendCopyBuyTransactionSimulated(_decodedInput, _transaction, _otherAccount, _localWeb3, _symbol, _otherTokenContract, _lastRunVariables, _nonce){
    console.log("Starting TX2", _symbol)
    return new Promise((resolve, reject) => {  

 const amountOutMin = _decodedInput.inputs[0];
 const path = _decodedInput.inputs[1];
 path[0] =  String(_localWeb3.utils.toChecksumAddress(path[0]))
 path[1] =  String(_localWeb3.utils.toChecksumAddress(path[1]))
 const to = Buffer.from(_otherAccount, 'hex') ; //May need to hex
 const deadline = _decodedInput.inputs[3];
   
 const routerV2Contract = new _localWeb3.eth.Contract(UniSwapV2RouterABI, UNISWAP_ROUTERV2_ADDRESS) 
 var approvalTxDataABI = routerV2Contract.methods.swapExactETHForTokens(String(amountOutMin), path, String(_otherAccount), deadline).encodeABI() //throws
 
 const txParams = {
    gas : _localWeb3.utils.toHex(_transaction.gas), //Copy original tickets gas limit
    gasPrice: _localWeb3.utils.toHex(String(parseInt(_transaction.gasPrice))), //Just bid slightly more than original ticket
    to: UNISWAP_ROUTERV2_ADDRESS,
    data: approvalTxDataABI,
    value: _localWeb3.utils.toHex(String(_transaction.value)),
    from: _otherAccount,
    nonce: _localWeb3.utils.toHex(_nonce)
  ...
  .once('receipt', function(receipt){
  console.log("**", _symbol,"TX2  Msg Receipt",  receipt.transactionHash)
   })
  .once('confirmation', async function(confirmationNumber, receipt){ 
  console.log("**", _symbol,"TX2 TX Confirmed", confirmationNumber, receipt.blockNumber, receipt.transactionHash)
  resolve()

  })// receipt)
  .on('error', async function(error){ 
  console.log("**", _symbol,"TX2 Error", error)// receipt)
  })
})}
...
async function createAndSendMySellTransactionSimulated(_decodedInput, _transaction, _myAccount, _localWeb3, _symbol, _lastRunVariables, _nonce){
   ...
   const txParams = {
    gas : _localWeb3.utils.toHex(300000), //Copy original tickets gas limit
    gasPrice: _localWeb3.utils.toHex(String(parseInt(_transaction.gasPrice))), //Just bid slightly less than original ticket
    to: UNISWAP_ROUTERV2_ADDRESS,
     data: approvalTxDataABI,
     value: _localWeb3.utils.toHex(newInputValue),
     from: _myAccount,
     nonce: _localWeb3.utils.toHex(_nonce)
   }
 ...
 async function approveERC20Spend(_addressOfToken, _addressOfSmartContractSpending, _accountPayingGas, _localWeb3, _transaction, _symbol, _nonce){
    ...
    const txParams = {
      gasPrice: _localWeb3.utils.toHex(approvalGasPrice), //30GWEI
      gas: _localWeb3.utils.toHex(80000), //80,000 units
